...
var dateTimeBox = grid.set(2, 8, 2, 4, blessed.box, makeBox(' ⏳ '));
tick();
...
function doTheTime() {
    ...
    dateTimeBox.content = '\n' + new Date().toDateString() + ' - ' + new Date().toLocaleTimeString();
    parrotBox.content = getAnsiArt('Hi! You\'re doing great!!! it\'s ' + new Date().toDateString());
    screen.render();
}
...
function getAnsiArt(textToSay) {
    ...
    switch (config.say) {
        case 'bunny' :
            return bunnySay(textToSay);
        case 'llama' :
            return llamaSay(textToSay);
        case 'cat'   :
            return catSay(textToSay);
        case 'yeoman':
            return yosay(textToSay);
        default :
            return ansiArt.get({artName: config.say, speechText: textToSay});
    }
}
